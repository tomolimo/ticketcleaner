...
function plugin_ticketcleaner_install() {
	...
	if (!TableExists("glpi_plugin_ticketcleaner_filters")) {
		$query = "CREATE TABLE `glpi_plugin_ticketcleaner_filters` (
						`id` INT(10) NOT NULL AUTO_INCREMENT,
						`type` INT(1) NOT NULL,
...
class PluginTicketCleaner {
	...
	public static function cleanText($parm) {
		...
		if( $is_content || $is_name ) { 
			...
			$filters = array( ) ;
			$query = "SELECT * FROM glpi_plugin_ticketcleaner_filters WHERE active=1 ORDER BY type, `order`;" ;
			...
			foreach ($DB->request($query) as $filter){
				if($filter['type'] == 0){
					if( array_key_exists('start', $filters ) )
						...
						$filters['start'] = '' ;
					$filters['start'] .= $filter['regex'] ;
				}
				elseif($filter['type'] == 1){
					if( array_key_exists('end', $filters ) )
						$filters['end'] .= '|' ;
					else
						$filters['end'] = '' ;
					$filters['end'] .= $filter['regex'] ;
				}
				elseif($filter['type'] == 2){
					$filters['replace'][] = array( 'regex' => $filter['regex'], 'replacement' => $filter['replacement'] ) ;
				}
				elseif($filter['type'] == 3){
					$filters['replace_in_title'][] = array( 'regex' => $filter['regex'], 'replacement' => $filter['replacement'] ) ;
				}
			}
		}			
			
		if( $is_content ) {
			// deletes signature from description field $parm->input['content']
			// deletes duplicated \r\n
			// deletes [cid:...]
					
			$ptn = '/('.$filters['start'].').*?('.$filters['end'].')/iu' ;

			// applies filter
			$parm->input['content'] = preg_replace( $ptn, '', $parm->input['content'] ) ;

			foreach($filters['replace'] as $ptn){
				$parm->input['content'] = preg_replace( '/'.$ptn['regex'].'/iu', $ptn['replacement'], $parm->input['content'] ) ;
			}
		}

		if( $is_name ) {
			foreach($filters['replace_in_title'] as $ptn){
				$parm->input['name'] = preg_replace( '/'.$ptn['regex'].'/iu', $ptn['replacement'], $parm->input['name'] ) ;
			}
		}
	}
